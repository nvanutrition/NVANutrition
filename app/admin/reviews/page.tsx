...
export default function AdminReviews() {
 ...
 if (loading) return <div className="p-8 text-foreground font-bold">Loading...</div>;
 ...
 <p className="text-muted-foreground mt-2 font-medium">Manage and moderate customer feedback.</p>
 </div>
 </motion.div>

 {/* Tabs */}
 <div className="flex gap-2 mb-8 bg-background/40 p-2 rounded-2xl w-fit border border-border">
 {['pending', 'approved', 'rejected'].map((tab) => (
 <button
 key={tab}
 onClick={() => setActiveTab(tab as any)}
 className={`px-5 py-2.5 rounded-xl text-sm font-bold capitalize transition-all duration-300 cursor-pointer ${
 activeTab === tab ? 'bg-green-500 text-black shadow-lg shadow-green-500/20' : 'text-muted-foreground hover:text-foreground hover:bg-muted'
 }`}
 >
 {tab}
 {tab === 'pending' && reviews.filter(r => r.status === 'pending').length > 0 && (
 <span className="ml-2 bg-red-500 text-foreground px-2 py-0.5 rounded-full text-xs">
 ...
 filtered.map(review => (
 <motion.div key={review.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass p-6 rounded-2xl border border-border flex flex-col md:flex-row gap-6 justify-between items-start">
 ...
 <span className="flex items-center gap-1.5"><User size={14} className="text-muted-foreground" /> {review.userName}</span>
