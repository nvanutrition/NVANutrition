...
export default function AdminSettings() {
  ...
  const Field = ({ label, name, type = 'text', placeholder = '' }: { label: string; name: keyof StoreSettings; type?: string; placeholder?: string }) => (
    <div>
      <label className="block text-sm font-bold text-gray-400 mb-2 uppercase tracking-wider">{label}</label>
      <input
        type={type}
        ...
        className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-600 focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition"
      />
    </div>
  );
    ...
    <div className="p-8 bg-gradient-dark min-h-screen">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">Configure your NVA Nutrition store preferences</p>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
      ...
      <div className="space-y-6 max-w-4xl">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-xl p-6 border border-white/10">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/10">
              <div>
                <p className="text-white font-semibold">Enable Announcement Ticker</p>
            ...
            <div>
              <label className="block text-sm font-bold text-gray-400 mb-2 uppercase tracking-wider">Ticker Message</label>
              <textarea
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-bold text-gray-400 mb-2 uppercase tracking-wider">Scroll Speed (px/s)</label>
                <input
                  type="number"
                  value={marquee.speed}
              ...
              <div>
                <label className="block text-sm font-bold text-gray-400 mb-2 uppercase tracking-wider">Background Color</label>
                <input
                  type="color"
                  value={marquee.bgColor}
                  onChange={e => setMarquee(prev => ({ ...prev, bgColor: e.target.value }))}
                  className="w-full h-[50px] bg-white/5 border border-white/10 rounded-xl p-1 focus:outline-none cursor-pointer"
                />
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-400 mb-2 uppercase tracking-wider">Text Color</label>
                <input
                  type="color"
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-xl p-6 border border-white/10">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-bold text-gray-400 mb-2 uppercase tracking-wider">Return & Refund Policy</label>
              <textarea
            ...
            <div>
              <label className="block text-sm font-bold text-gray-400 mb-2 uppercase tracking-wider">Shipping & Delivery Policy</label>
              <textarea
            ...
            <div>
              <label className="block text-sm font-bold text-gray-400 mb-2 uppercase tracking-wider">Terms & Conditions</label>
              <textarea
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-xl p-6 border border-white/10">
          ...
          <div className="mt-4">
            <label className="block text-sm font-bold text-gray-400 mb-2 uppercase tracking-wider">Low Stock Threshold (units)</label>
            <input
              type="number"
              min={1}
              value={settings.lowStockThreshold}
              onChange={e => setSettings(prev => ({ ...prev, lowStockThreshold: parseInt(e.target.value) || 10 }))}
              className="w-32 px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition font-mono font-bold"
            />
          </div>
        </motion.div>
      </div>
