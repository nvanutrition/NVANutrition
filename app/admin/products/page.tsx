    ...
    <div className="min-h-screen bg-gradient-dark p-6 space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-2.5 glass border border-white/10 rounded-xl text-gray-400 hover:text-white transition cursor-pointer"
            title="Refresh"
          >
            <RefreshCw size={18} />
          </button>
          {activeTab === 'storefront' ? (
            ...
            <motion.button
              ...
              className="flex items-center gap-2 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white px-5 py-2.5 rounded-xl font-bold shadow-lg shadow-blue-500/20 cursor-pointer text-sm"
            >
      ...
      <div className="flex gap-2 p-1 glass rounded-xl border border-white/10 w-fit">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition ${activeTab === 'storefront' ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'text-gray-400 hover:text-white'}`}
        >
          <Package size={16} /> Storefront Products
        </button>
        <button
          onClick={() => setActiveTab('free_gifts')}
          className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition ${activeTab === 'free_gifts' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' : 'text-gray-400 hover:text-white'}`}
        >
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-black/30 border border-white/10 text-white placeholder:text-gray-600 rounded-xl focus:outline-none focus:border-green-500 transition text-sm"
              />
            </div>
            {/* Filter toggle */}
            <button
              onClick={() => setShowFilters(v => !v)}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-semibold transition cursor-pointer ${showFilters ? 'bg-green-500/20 border-green-500/40 text-green-400' : 'border-white/10 text-gray-400 hover:text-white'}`}
            >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {CATEGORIES.map(cat => (
                        <button key={cat} onClick={() => setCategoryFilter(cat)}
                          className={`px-3 py-1 rounded-lg text-xs font-semibold border transition cursor-pointer ${categoryFilter === cat ? 'bg-green-500/20 border-green-500/40 text-green-400' : 'border-white/10 text-gray-400 hover:text-white'}`}>
                          {cat}
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {STOCK_FILTERS.map(sf => (
                        <button key={sf} onClick={() => setStockFilter(sf)}
                          className={`px-3 py-1 rounded-lg text-xs font-semibold border transition cursor-pointer ${stockFilter === sf ? 'bg-green-500/20 border-green-500/40 text-green-400' : 'border-white/10 text-gray-400 hover:text-white'}`}>
                          {sf}
                        </button>
                      ))}
                    </div>
      ...
      {loading ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          className="text-center py-20 glass rounded-xl border-2 border-dashed border-white/10">
          <Package size={44} className="mx-auto text-gray-600 mb-3" />
          <p className="text-gray-400 text-base font-semibold">No products found</p>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="px-5 py-3 border-b border-white/10 flex items-center justify-between">
            <span className="text-sm text-gray-400">
              Showing <span className="text-white font-bold">{filtered.length}</span> of <span className="text-white font-bold">{products.length}</span> products
            </span>
          </div>
                ...
                <tr className="border-b border-white/10 bg-white/3">
                  ...
                  {([
                    ...
                    <th key={field} className="px-4 py-3 text-left cursor-pointer group" onClick={() => toggleSort(field)}>
                      <span className="text-xs font-bold text-gray-500 uppercase tracking-wider group-hover:text-gray-300 transition">
                  ...
                  {filtered.map((product, idx) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="w-12 h-12 rounded-lg overflow-hidden bg-white/5 border border-white/10 flex items-center justify-center flex-shrink-0">
                          {product.images?.[0] ? (
                      ...
                      {/* Category */}
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-white/8 text-gray-300 text-xs font-semibold border border-white/10">
                          <Tag size={10} />
                          {product.category}
                        </span>
                      </td>
          ...
          {/* Table footer */}
          <div className="px-5 py-3 border-t border-white/10 flex justify-between items-center text-xs text-gray-600">
        ...
        {showAddGift && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              className="bg-[#0a0f0a] border border-white/10 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
              <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between">
                <h3 className="text-lg font-bold text-white flex items-center gap-2"><Gift className="text-blue-400" size={18} /> Add Free Gift</h3>
                <button onClick={() => setShowAddGift(false)} className="text-gray-500 hover:text-white transition"><XCircle size={20} /></button>
              </div>
              <form onSubmit={handleSaveGift} className="p-6 space-y-4">
                <div>
                  <label className="text-xs font-semibold text-gray-400 block mb-1">Gift Name <span className="text-red-400">*</span></label>
                  <input type="text" required value={giftForm.name} onChange={e => setGiftForm(p => ({ ...p, name: e.target.value }))}
                    className="w-full px-4 py-2.5 bg-black/30 border border-white/10 text-white placeholder:text-gray-600 rounded-xl focus:outline-none focus:border-blue-500/60 focus:ring-1 focus:ring-blue-500/20 text-sm"
                    placeholder="e.g. Free Shaker Bottle" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-400 block mb-1">Gift Image</label>
                  {!giftFile ? (
                    <label className="flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-white/15 hover:border-blue-500/40 rounded-xl cursor-pointer bg-black/20 hover:bg-black/30 transition group">
                      <Upload size={20} className="text-gray-500 group-hover:text-blue-400 transition mb-1" />
                      <span className="text-xs font-semibold text-gray-500 group-hover:text-gray-300 transition">Click to upload image</span>
                    ...
                    <div className="relative w-24 h-24 rounded-xl overflow-hidden border border-blue-500/30 group">
                      <img src={giftFile.preview} alt="Preview" className="w-full h-full object-contain bg-white/5" />
                      <button type="button" onClick={() => setGiftFile(null)} className="absolute top-1 right-1 p-1 bg-red-500 rounded-full text-white opacity-0 group-hover:opacity-100 transition shadow-lg cursor-pointer">
                        <X size={12} />
                      </button>
                    </div>
                  )}
                ...
                <div>
                  <label className="text-xs font-semibold text-gray-400 block mb-1">Available Stock <span className="text-red-400">*</span></label>
                  <input type="number" required min={0} value={giftForm.stock} onChange={e => setGiftForm(p => ({ ...p, stock: e.target.value }))}
                    className="w-full px-4 py-2.5 bg-black/30 border border-white/10 text-white rounded-xl focus:outline-none focus:border-blue-500/60 focus:ring-1 focus:ring-blue-500/20 text-sm" />
                </div>
                <div className="pt-4 flex gap-3">
                  <button type="button" onClick={() => setShowAddGift(false)}
                    className="flex-1 px-4 py-2.5 rounded-xl border border-white/10 text-gray-400 hover:text-white transition font-semibold text-sm">Cancel</button>
                  <button type="submit" disabled={savingGift}
                    className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 disabled:opacity-50 text-white rounded-xl font-bold text-sm shadow-lg shadow-blue-500/20 transition">
