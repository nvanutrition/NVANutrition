...
const inputCls = "w-full px-4 py-2.5 bg-black/30 border border-white/10 text-white placeholder:text-gray-600 rounded-xl focus:outline-none focus:border-green-500/60 focus:ring-1 focus:ring-green-500/20 transition text-sm";
const selectCls = "w-full px-4 py-2.5 bg-black/30 border border-white/10 text-white rounded-xl focus:outline-none focus:border-green-500/60 transition text-sm";
const textareaCls = "w-full px-4 py-2.5 bg-black/30 border border-white/10 text-white placeholder:text-gray-600 rounded-xl focus:outline-none focus:border-green-500/60 focus:ring-1 focus:ring-green-500/20 transition text-sm resize-none";
    ...
    <div className="sticky top-6 space-y-4">
      ...
      <div className="rounded-2xl overflow-hidden border border-white/15 bg-[#0d1117] shadow-2xl">
        ...
        <div className="relative bg-gradient-to-br from-white/5 to-white/[0.02] h-52 flex items-center justify-center overflow-hidden">
          ...
          {discount > 0 && (
            <div className="absolute top-3 left-3 bg-green-600 text-white text-xs font-black px-2.5 py-1 rounded-lg shadow">
              {discount}% OFF
            </div>
          )}
          {data.isFeatured && (
          ...
          {data.flavors && (
            <div className="flex flex-wrap gap-1.5">
              {data.flavors.split(',').map(f => f.trim()).filter(Boolean).slice(0, 4).map(f => (
                <span key={f} className="px-2 py-0.5 rounded-full bg-white/8 text-gray-300 text-[10px] font-semibold border border-white/10">{f}</span>
              ))}
            </div>
          )}
          ...
          {data.nutritionOptions.length > 0 && (
            <div className="grid grid-cols-2 gap-1.5 pt-1">
              {data.nutritionOptions.slice(0, 4).map((n, i) => (
                <div key={i} className="bg-white/5 rounded-lg px-2.5 py-1.5 text-center border border-white/8">
                  <p className="text-white text-xs font-black">{n.quantity}{n.unit}</p>
  ...
  return (
    <div className="min-h-screen bg-gradient-dark">
      <form onSubmit={handleSave}>
        {/* ─── Sticky Top Bar ─── */}
        <div className="sticky top-0 z-30 bg-[#0a0f0a]/90 backdrop-blur-xl border-b border-white/10 px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={() => router.push('/admin/products')}
              className="px-4 py-2 rounded-xl border border-white/10 text-gray-400 hover:text-white hover:bg-white/5 transition text-sm font-semibold cursor-pointer"
            >
              Cancel
            </button>
            <motion.button
        ...
        <div className="p-6 flex gap-6 items-start max-w-[1400px] mx-auto">
          ...
          <div className="flex-1 min-w-0 space-y-5">
            ...
            {/* Tab strip */}
            <div className="flex overflow-x-auto gap-1 p-1 glass rounded-2xl border border-white/10 no-scrollbar">
              {TABS.map(tab => (
                <button
                  key={tab.id}
                  type="button"
            ...
            <div className="flex items-center gap-2 text-xs text-gray-600 bg-white/3 border border-white/8 rounded-xl px-4 py-2.5">
              ...
              <span><strong className="text-gray-400">{TABS.find(t => t.id === activeTab)?.label}:</strong> {TABS.find(t => t.id === activeTab)?.hint}</span>
            </div>
            ...
            <AnimatePresence mode="wait">
              {activeTab === 'basic' && (
                <motion.div key="basic" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                  ...
                  <h2 className="text-white font-bold text-base flex items-center gap-2"><Tag size={16} className="text-green-400" /> Basic Information</h2>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    {/* Weight row */}
                    <div>
                      <label className="text-sm font-semibold text-gray-300 block mb-1.5">Product Weight / Size</label>
                      <div className="flex gap-2">
                        ...
                        <select className="w-24 px-4 py-2.5 bg-black/30 border border-white/10 text-white rounded-xl focus:outline-none focus:border-green-500/60 transition text-sm flex-shrink-0" value={form.weightUnit} onChange={e => up({ weightUnit: e.target.value })}>
                    ...
                    <div className="flex items-end">
                      <label className="flex items-center gap-3 cursor-pointer select-none">
                        <div
                          ...
                          className={`relative w-12 h-6 rounded-full transition-colors cursor-pointer ${form.isFeatured ? 'bg-green-500' : 'bg-white/10'}`}
                        >
                        ...
                        <div>
                          <p className="text-white font-semibold text-sm">Mark as Featured</p>
              ...
              {activeTab === 'pricing' && (
                <motion.div key="pricing" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                  className="glass rounded-2xl border border-white/10 p-6 space-y-5">
                  <h2 className="text-white font-bold text-base flex items-center gap-2"><DollarSign size={16} className="text-green-400" /> Pricing & Stock</h2>
              ...
              {activeTab === 'content' && (
                <motion.div key="content" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                  className="glass rounded-2xl border border-white/10 p-6 space-y-5">
                  <h2 className="text-white font-bold text-base flex items-center gap-2"><FileText size={16} className="text-green-400" /> Content & Descriptions</h2>
              ...
              {activeTab === 'nutrition' && (
                <motion.div key="nutrition" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                  ...
                  <div className="flex items-center justify-between">
                    <h2 className="text-white font-bold text-base flex items-center gap-2"><Zap size={16} className="text-green-400" /> Nutrition Macros</h2>
                    <button type="button"
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    ...
                    {[
                      ...
                      <button key={m.name} type="button"
                        ...
                        className="px-3 py-1 text-xs rounded-lg bg-white/5 border border-white/10 text-gray-400 hover:text-green-400 hover:border-green-500/30 transition cursor-pointer">
                        + {m.name}
                      </button>
                    ))}
                  </div>
              ...
              {activeTab === 'ingredients' && (
                <motion.div key="ingredients" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                  className="glass rounded-2xl border border-white/10 p-6 space-y-5">
                  <div className="flex items-center justify-between">
                    <h2 className="text-white font-bold text-base flex items-center gap-2"><List size={16} className="text-green-400" /> Key Highlighted Ingredients</h2>
                    <button type="button"
              ...
              {activeTab === 'images' && (
                <motion.div key="images" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                  className="glass rounded-2xl border border-white/10 p-6 space-y-5">
                  <h2 className="text-white font-bold text-base flex items-center gap-2"><ImageIcon size={16} className="text-green-400" /> Product Images</h2>

                  {/* Upload area */}
                  <label className="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-white/15 hover:border-green-500/40 rounded-2xl cursor-pointer bg-black/20 hover:bg-black/30 transition group">
                    <Upload size={24} className="text-gray-500 group-hover:text-green-400 transition mb-2" />
                    <span className="text-sm font-semibold text-gray-500 group-hover:text-gray-300 transition">Click to upload images</span>
                  ...
                  {/* URL paste row */}
                  <div>
                    <label className="text-sm font-semibold text-gray-400 block mb-2">Or paste an image URL</label>
                    <div className="flex gap-2">
                      ...
                      />
                      <button type="button"
                        className="px-4 py-2.5 bg-white/10 hover:bg-white/15 text-white rounded-xl text-sm font-semibold transition cursor-pointer border border-white/10"
                        onClick={e => {
                        ...
                        {form.images.map((img, idx) => (
                          <div key={`existing-${idx}`} className="relative aspect-square rounded-xl overflow-hidden bg-white/5 border border-white/10 group">
                            ...
                            <button type="button"
                              ...
                              className="absolute top-1.5 right-1.5 p-1 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition cursor-pointer shadow">
                              <X size={11} />
                            </button>
                          </div>
                        ))}
                        {selectedFiles.map((f, idx) => (
                          <div key={`new-${idx}`} className="relative aspect-square rounded-xl overflow-hidden bg-white/5 border border-green-500/30 group">
            ...
            <div className="glass rounded-2xl border border-white/10 p-4 flex items-center justify-between">
              ...
              <div className="flex gap-3">
                <button type="button" onClick={() => router.push('/admin/products')}
                  className="px-5 py-2.5 rounded-xl border border-white/10 text-gray-400 hover:text-white text-sm font-semibold transition cursor-pointer">
                  Cancel
                </button>
