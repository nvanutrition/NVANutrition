...
export default function AdminOffers() {
 ...
 return (
 <div className="p-8 bg-gradient-dark min-h-screen text-foreground">
 {/* Header */}
 ...
 <p className="text-muted-foreground mt-1">Manage rules, buy-one-get-ones, and cart discounts</p>
 </div>
 {!showForm && (
 <button
 ...
 className="glass border border-border rounded-2xl p-6 mb-8 max-w-4xl"
 >
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Offer Name</label>
 <input
 type="text"
 placeholder="e.g. Buy 2 Get 1 Free Creatine"
 value={formData.name}
 onChange={e => setFormData({ ...formData, name: e.target.value })}
 className="w-full px-4 py-3 bg-muted border border-border rounded-xl focus:outline-none focus:border-green-500"
 />
 </div>

 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Offer Status</label>
 <select
 value={formData.status}
 onChange={e => setFormData({ ...formData, status: e.target.value as any })}
 className="w-full px-4 py-3 bg-muted border border-border rounded-xl focus:outline-none focus:border-green-500 cursor-pointer"
 >
 <option value="draft" className="bg-muted/30">Draft / Inactive</option>
 <option value="live" className="bg-muted/30">Live / Active</option>
 </select>
 </div>

 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Promotion Type</label>
 <select
 value={formData.offerType}
 onChange={e => setFormData({ ...formData, offerType: e.target.value as any })}
 className="w-full px-4 py-3 bg-muted border border-border rounded-xl focus:outline-none focus:border-green-500 cursor-pointer"
 >
 <option value="percentage_discount" className="bg-muted/30">% Percentage Discount</option>
 <option value="flat_discount" className="bg-muted/30">Flat Amount Off</option>
 <option value="free_product" className="bg-muted/30">Add Free Gift Product</option>
 <option value="bxgy" className="bg-muted/30">Buy X Get Y Free (BXGY)</option>
 </select>
 </div>

 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Min Cart Total (₹)</label>
 <input
 type="number"
 placeholder="e.g. 2999"
 value={formData.minCartValue}
 onChange={e => setFormData({ ...formData, minCartValue: e.target.value })}
 className="w-full px-4 py-3 bg-muted border border-border rounded-xl focus:outline-none"
 />
 </div>
 </div>
 ...
 <div className="p-4 bg-muted rounded-xl border border-border grid grid-cols-1 md:grid-cols-2 gap-4">
 <div className="md:col-span-2 text-xs font-black text-green-400 uppercase tracking-wide">
 Optional SKU Trigger Conditions
 </div>
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Requires Specific Product SKU</label>
 <select
 value={formData.targetSku}
 onChange={e => setFormData({ ...formData, targetSku: e.target.value })}
 className="w-full px-4 py-2 bg-muted border border-border rounded-lg text-sm cursor-pointer focus:outline-none focus:border-green-500"
 >
 <option value="" className="bg-muted/30">-- Select Target SKU --</option>
 {products.map(p => (
 <option key={p.id} value={p.sku || p.id} className="bg-muted/30">
 {p.name} ({p.sku || 'No SKU'})
 </option>
 ))}
 </select>
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Min Quantity of SKU</label>
 <input
 type="number"
 placeholder="e.g. 2"
 ...
 className="w-full md:w-1/2 px-4 py-2.5 bg-muted border border-border rounded-lg"
 />
 </div>
 )}
 ...
 <div className="p-4 bg-muted rounded-xl border border-border grid grid-cols-1 md:grid-cols-2 gap-4">
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Reward Gift SKU</label>
 <select
 value={formData.rewardSku}
 onChange={e => setFormData({ ...formData, rewardSku: e.target.value })}
 className="w-full px-4 py-2 bg-muted border border-border rounded-lg text-sm cursor-pointer focus:outline-none focus:border-green-500"
 >
 <option value="" className="bg-muted/30">-- Select Reward Gift SKU --</option>
 {products.map(p => (
 <option key={p.id} value={p.sku || p.id} className="bg-muted/30">
 {p.name} ({p.sku || 'No SKU'})
 </option>
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Gift Quantity</label>
 <input
 type="number"
 placeholder="1"
 ...
 {formData.offerType === 'bxgy' && (
 <div className="p-4 bg-muted rounded-xl border border-border grid grid-cols-1 md:grid-cols-4 gap-4">
 <div className="md:col-span-4 text-xs font-black text-green-400 uppercase tracking-wide">
 Buy X Get Y Setup
 </div>
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Buy SKU</label>
 <select
 value={formData.buySku}
 onChange={e => setFormData({ ...formData, buySku: e.target.value })}
 className="w-full px-4 py-2 bg-muted border border-border rounded-lg text-sm cursor-pointer focus:outline-none focus:border-green-500"
 >
 <option value="" className="bg-muted/30">-- Select Buy SKU --</option>
 {products.map(p => (
 <option key={p.id} value={p.sku || p.id} className="bg-muted/30">
 {p.name} ({p.sku || 'No SKU'})
 </option>
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Buy Min Qty</label>
 <input
 type="number"
 placeholder="2"
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Get SKU (Free)</label>
 <select
 value={formData.getSku}
 onChange={e => setFormData({ ...formData, getSku: e.target.value })}
 className="w-full px-4 py-2 bg-muted border border-border rounded-lg text-sm cursor-pointer focus:outline-none focus:border-green-500"
 >
 <option value="" className="bg-muted/30">-- Select Free Get SKU --</option>
 {products.map(p => (
 <option key={p.id} value={p.sku || p.id} className="bg-muted/30">
 {p.name} ({p.sku || 'No SKU'})
 </option>
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Get Free Qty</label>
 <input
 type="number"
 placeholder="1"
 ...
 type="button"
 onClick={resetForm}
 className="px-5 py-3 bg-muted hover:bg-muted text-foreground rounded-xl font-bold transition cursor-pointer text-sm"
 >
 Cancel
 </button>
 <button
 ...
 <thead>
 <tr className="border-b border-border bg-muted text-muted-foreground text-xs font-bold uppercase tracking-wider">
 ...
 offers.map(offer => (
 <tr key={offer.id} className="hover:bg-muted transition">
 <td className="px-6 py-4 font-bold text-foreground text-sm">{offer.name}</td>
 ...
 <span className="flex items-center gap-1.5 px-3 py-1 bg-gray-500/10 border border-gray-500/20 text-muted-foreground text-xs font-extrabold rounded-full">
 Draft
 </span>
 )}
 </button>
 ...
 className="p-2 bg-muted border border-border hover:border-green-500 hover:text-green-500 rounded-lg transition cursor-pointer"
 >
 <Edit size={16} />
 </button>
 <button
 onClick={() => handleDelete(offer.id)}
 className="p-2 bg-muted border border-border hover:border-red-500 hover:text-red-500 rounded-lg transition cursor-pointer"
 >
 <Trash2 size={16} />
 </button>
 </div>
